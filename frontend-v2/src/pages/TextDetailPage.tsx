...
import { TextData, VocabularyItem, TextAnalysisData } from '@usuaya/shared-types';
...
function TextDetailPage() {
  ...
  const renderSpanishText = () => {
      ...
      if (!wordTimings || !spanishWordMap) return <Typography color="error">Analysis data missing for Spanish text.</Typography>;
        ...
        <Box my={3} p={2} bgcolor="#e9ecef" borderRadius={2} boxShadow="inset 0 1px 2px rgba(0,0,0,0.075)">
          ...
          <Box component="ul" sx={{ listStyle: 'disc', pl: 3 }}> {/* Use Box for list styling */}
            {text.original_vocabulary.map((item: VocabularyItem) => (
              <Typography component="li" key={item.id} variant="body2" sx={{ mb: 0.5 }}>{item.word}</Typography>
            ))}
          </Box>
        </Box>
      )}
