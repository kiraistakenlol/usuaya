...
import { TextData, VocabularyItem } from '@usuaya/shared-types'; // Phrase type isn't explicitly needed if we handle response generically

// Helper component for relative time (can be moved to components later)
// Using a simple implementation for now
function SimpleRelativeTimeDisplay({ dateString }: { dateString: string | null | undefined }) {
  if (!dateString) return null;
  try {
    const date = new Date(dateString);
    // Basic relative time logic (replace with more robust library/logic if needed)
    const diffSeconds = Math.round((Date.now() - date.getTime()) / 1000);
    if (diffSeconds < 60) return <Typography variant="caption" color="text.secondary">just now</Typography>;
    if (diffSeconds < 3600) return <Typography variant="caption" color="text.secondary">{Math.round(diffSeconds / 60)}m ago</Typography>;
    if (diffSeconds < 86400) return <Typography variant="caption" color="text.secondary">{Math.round(diffSeconds / 3600)}h ago</Typography>;
    return <Typography variant="caption" color="text.secondary">{date.toLocaleDateString()}</Typography>;
  } catch (e) {
    return null;
  }
}
            ...
            <Grid item xs={12} md={6}>
              <Paper variant="outlined" sx={{ p: 2, bgcolor: 'grey.50', height: '300px' }}>
                ...
                {loadingVocab ? (
                  ...
                  <Paper variant="outlined" sx={{ height: 'calc(100% - 30px)', overflowY: 'auto', bgcolor: 'white' }}>
                    {vocabulary.length === 0 ? (
          ...
          <List disablePadding>
            {texts.map((text, index) => (
              <React.Fragment key={text.id}>
                <ListItemButton component={RouterLink} to={`/texts/${text.id}`} sx={{ p: 2, width: '100%' }}>
                  <Box sx={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <Typography variant="body1" component="h3" sx={{ flexGrow: 1, mr: 2, fontWeight: 500 }} noWrap>
                         {text.spanish_text || `Text ${text.id}`}{/* Display spanish_text or fallback */}
                      </Typography>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexShrink: 0 }}>
                        <SimpleRelativeTimeDisplay dateString={text.created_at} />
