...
echo "Checking for existing frontend-v2 process..."
...
echo "Starting frontend-v2 in foreground (localhost:$FRONTEND_PORT)..."
(
  # Change directory to frontend-v2
  cd frontend-v2 || exit 1 
  # Check if node_modules exists, if not install dependencies
  if [ ! -d "node_modules" ]; then
      echo "Node modules not found in frontend-v2/. Installing dependencies..."
  ...
  # npm run dev in frontend-v2 executes "vite". The -- --port passes the argument.
...
echo "Frontend-v2 process finished."
